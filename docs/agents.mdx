...
Subcommands drill into specific surfaces. Below: the dashboard itself (`/agents`), then live tail of agent stdout (`/agents trace`), then the cross-agent context bus (`/agents bus`).
...
`cached_input_tokens` and `cache_creation_input_tokens` (Anthropic)
and `cached_input_tokens` / `reasoning_output_tokens` (Codex) are
excluded from the count — they bill at different rates and a flat
sum would mis-price the `$/hr` column. A follow-up issue can split
the meters' return surface so cache and reasoning tokens are priced
correctly.
...
`usd_per_token` is a 70/30 input/output blend (rationale in
`app/agents/pricing.py`). Pricing returns `None` for unknown models
and the cell falls back to `-`; the dashboard never invents a rate.
The yaml `hourly_budget_usd` field is **not** the cell content —
it's reserved for a future budget-alarm feature.
